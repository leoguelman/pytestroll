    ...
    def nht_size_nn(self):
        ...
        if self.s.size == 1:
            if self.N is None:
                ...
                out = (z_alpha + z_beta)**2 * (2 * self.s**2) * self.N /                          \
                      (self.d**2 * (self.N-1) + (z_alpha + z_beta)**2 * 4 * self.s**2)
            ...
            if self.N is None:
                ...
                n1 = (z_alpha + z_beta)**2 * self.N * (self.s[0]**2 + self.s[0]*self.s[1]) /            \
                     (self.d**2 * (self.N-1) + (z_alpha + z_beta)**2 * (self.s[0] + self.s[1])**2)
                n2 = (z_alpha + z_beta)**2 * self.N * (self.s[1]**2 + self.s[0]*self.s[1]) /        \
                     (self.d**2 * (self.N-1) + (z_alpha + z_beta)**2 * (self.s[0] + self.s[1])**2)
...
class TestRoll:
    ...
    def tr_size_nn(self):
        ...
        if self.s.size == 1: # symmetric 
            ...
            self.sample_size = np.repeat(self.n, 2, axis=0)
    ...
    def profit_nn(self, n:float = None, log_n:bool = False, sign:float = 1.0):
        ...
        res = self.sign * (deploy + test)/self.N
    ...
    def test_eval_nn(self, n:float = None, R:int = None):
